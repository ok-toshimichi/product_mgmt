...
class exeUpdate extends Controller
{
    /**
     * 商品情報の更新
     *
    ...
    public function __invoke(ProductRequest $request) {
        ...
        // 箱  ： $product_instanceいう名前の変数(function同様に、中身が分かるものがよい)
        ...
        // 箱  ： $update_dataいう名前の変数(function同様に、中身が分かるものがよい)
        // 中身: $registerproductdata_instanceに$requestを渡してあげる。渡った$requestをregisterProductData.php側で加工する。
        $update_data = $registerproductdata_instance($request);
